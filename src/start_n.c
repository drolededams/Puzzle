		...
		t_puzzle_data *data)
{
	...
	while (heap->size > 0 && !success)
	{
		count++;
		state = pop_heap(heap);
		if (state->id == id_goal && same_state(state->coor, data->goal_coor,
					data->area))
			success = 1;
		else
		{
			...
			while (++i < 4)
			{
				if ((id_successor = data->move_n[i](state, su_coor, su_val,
								data->size)))
				{
					if (!(successor = search_node_n(hash_tab[
									hash_table_n(su_coor, su_val, data->area)],
									id_successor, su_coor, data->area)))
					{
						count_2++;
						successor = mem_state_n(id_successor, su_coor, state,
								data);
